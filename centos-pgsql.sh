...
yum -y install unixODBC unixODBC-devel libtool-ltdl libtool-ltdl-devel postgresql-odbc postgresql postgresql-devel
...
cd ..
cd ..
cp -rf asterisk/conf/* /etc/asterisk/
...
mv aplication/virtualskynet.war  /root/virtualskynet/webapps/ROOT.war

echo -e "\e[1;31m  -  Installing Last Database Schema  -  \e[0m"
/usr/pgsql-9.2/bin/pg_restore  -U postgres  -C -c -d postgres  aplication/virtualskynet.sql
